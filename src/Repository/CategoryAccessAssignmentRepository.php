...
final class CategoryAccessAssignmentRepository implements CategoryAccessAssignmentRepositoryInterface
{
    ...
    public function save(CategoryAccessAssignmentInterface $assignment): void
    {
        ...
        $existing = $this->connection->fetchOne(
            'SELECT assignment_id FROM category_access_assignment WHERE category_id = :categoryId AND actor_user_id = :actorUserId',
            ['categoryId' => $assignment->categoryId(), 'actorUserId' => $assignment->actorUserId()],
    ...
    public function findActiveByCategoryId(string $categoryId): array
    {
        ...
        return $this->hydrateMany(
            $this->connection->fetchAllAssociative(
                'SELECT assignment_id, category_id, actor_user_id, role, status, is_primary, granted_at, revoked_at FROM category_access_assignment WHERE category_id = :categoryId AND status = :status ORDER BY is_primary DESC, granted_at ASC',
    ...
    public function findActiveByActorUserId(string $actorUserId): array
    {
        ...
        return $this->hydrateMany(
            $this->connection->fetchAllAssociative(
                'SELECT assignment_id, category_id, actor_user_id, role, status, is_primary, granted_at, revoked_at FROM category_access_assignment WHERE actor_user_id = :actorUserId AND status = :status ORDER BY granted_at ASC',
    ...
    public function findOneByCategoryIdAndActorUserId(string $categoryId, string $actorUserId): ?CategoryAccessAssignmentInterface
    {
        ...
        $row = $this->connection->fetchAssociative(
            'SELECT assignment_id, category_id, actor_user_id, role, status, is_primary, granted_at, revoked_at FROM category_access_assignment WHERE category_id = :categoryId AND actor_user_id = :actorUserId LIMIT 1',
