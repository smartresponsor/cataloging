...
final class CategoryProjectionSync implements CategoryProjectionSyncInterface
{
    ...
    public function apply(array $event): void
    {
        ...
        $row = $this->dataConnection()->fetchAssociative(
            'SELECT id, slug, name, parent_id, path, locale, tenant, workflow_state, published, published_at FROM category WHERE id = :id LIMIT 1',
            ['id' => $categoryId],
        ...
        $this->infraConnection()->executeStatement(
            'INSERT INTO category_projection (id, slug, name, parent_id, path, locale, tenant, workflow_state, published, published_at, updated_at) '
            .'VALUES (:id, :slug, :name, :parentId, :path, :locale, :tenant, :workflowState, :published, :publishedAt, :updatedAt) '
            .'ON DUPLICATE KEY UPDATE slug = VALUES(slug), name = VALUES(name), parent_id = VALUES(parent_id), path = VALUES(path), locale = VALUES(locale), tenant = VALUES(tenant), workflow_state = VALUES(workflow_state), published = VALUES(published), published_at = VALUES(published_at), updated_at = VALUES(updated_at)',
            [
            ...
            [
                ...
                'parentId' => null === $parentId ? ParameterType::NULL : ParameterType::STRING,
                ...
                'publishedAt' => null === $publishedAt ? ParameterType::NULL : ParameterType::STRING,
