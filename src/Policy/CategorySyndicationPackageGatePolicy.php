...
final class CategorySyndicationPackageGatePolicy implements CategorySyndicationPackageGatePolicyInterface
{
    public function buildReport(array $packageMissingRequiredFields, array $mediaRequiredMissing, array $warnings, array $mediaChecks, array $matchedBindingIds): CategorySyndicationPackageGateReportInterface
    {
        ...
        $mediaRequiredMissing = array_values(array_unique(array_filter(array_map(static fn (mixed $value): string => trim((string) $value), $mediaRequiredMissing), static fn (string $value): bool => '' !== $value)));
        $warnings = array_values(array_unique(array_filter(array_map(static fn (mixed $value): string => trim((string) $value), $warnings), static fn (string $value): bool => '' !== $value)));
        $matchedBindingIds = array_values(array_unique(array_filter(array_map(static fn (mixed $value): string => trim((string) $value), $matchedBindingIds), static fn (string $value): bool => '' !== $value)));
        ...
        $checks = [
            ...
            'destinationMediaReady' => [] === $mediaRequiredMissing && (bool) ($mediaChecks['destinationMediaPublishable'] ?? false),
