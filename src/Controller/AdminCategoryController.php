...
final class AdmintestsController extends AbstractController
{
    ...
    public function previewMove(Request $req, EntityManagerInterface $em): JsonResponse
    {
        ...
        /** @var testsEntity|null $source */
        $source = $em->getRepository(testsEntity::class)->find($sourceId);
        /** @var testsEntity|null $target */
        $target = $em->getRepository(testsEntity::class)->find($targetId);
        if (!$source || !$target) {
        ...
        $dup = $em->getRepository(testsEntity::class)->findOneBy(['slug' => $source->getSlug(), 'depth' => $newDepth]);
