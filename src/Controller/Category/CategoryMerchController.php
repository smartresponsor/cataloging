...
final class CategoryMerchController extends AbstractController
{
        ...
        public function pinCreate(string $id, Request $r): JsonResponse
    {
        ...
        $this->em->persist($pin); $this->em->flush();
        ...
        public function pinDelete(string $id, Request $r): JsonResponse
    {
        ...
        if ($pin) { $this->em->remove($pin); $this->em->flush(); }
        return $this->json(['ok'=>true]);
    }
    ...
    {
        $title = (string)$r->request->get('title'); $content = (string)$r->request->get('content');
        $b = new CategoryBanner($id, $title, $content); $b->publish();
        $this->em->persist($b); $this->em->flush();
        ...
        public function htmlPublish(string $id, Request $r): JsonResponse
    {
        ...
        $h = new CategoryHtmlBlock($id, $html); $h->publish();
