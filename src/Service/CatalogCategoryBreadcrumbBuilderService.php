 ...
 */
final readonly class CatalogCategoryBreadcrumbBuilderService implements CatalogCategoryBreadcrumbBuilderServiceInterface
{
    /**
    ...
    private function normalizeChain(mixed $value): array
    {
        ...
        foreach ($value as $node) {
            ...
            $id = $node['id'] ?? null;
            $name = $node['name'] ?? $node['nameEntity'] ?? null;
            $slug = $node['slug'] ?? null;
