...
final class SearchService
{
    ...
    /**
     * @param array{q:string,tenant:?string,locale:?string,workflow_state:?string,published:?bool,limit:int,offset:int,order:string,direction:string} $criteria
     ...
     * @return list<array{id:string,slug:string,name:string,parent_id:?string,path:string,locale:string,tenant:string,workflow_state:string,published:bool,published_at:?string,updated_at:string}>
     */
     ...
     */
    private function facetCounts(Connection $connection, string $field, string $whereSql, array $params, array $types): array
    {
        ...
        $rows = $connection->fetchAllAssociative(
            sprintf('SELECT %1$s AS facet_value, COUNT(*) AS facet_count FROM category_projection%2$s GROUP BY %1$s ORDER BY facet_count DESC, %1$s ASC', $field, $whereSql),
            $params,
            $types,
        );
