...
set "COMMANDING_SH=.commanding/command.sh"

REM Detect Windows Terminal
set "WT=0"
if defined WT_SESSION set "WT=1"

REM Detect Git Bash candidates
set "BASH="
for %%P in (
...
if defined BASH (
  if "%WT%"=="1" (
    ...
    start "" /min wt -w 0 new-tab --title "%REPO_NAME%" -d "%REPO_DIR%" ^
      -- "%BASH%" -lc "%BASH_CMD%" >nul 2>nul
    exit /b 0
  ) else (
