 ...
 * Enforces canonical Cataloging/Catalog directory rules for src/ and tests/.
 */
$root = $argv[1] ?? getcwd();
...
foreach ($scanRoots as $scanRoot) {
    ...
    foreach ($iter as $item) {
        ...
        if (preg_match('~^src/.+/(Catalog|Cataloging)(/|$)~', $relative) === 1) {
            fwrite(STDERR, "Forbidden nested namespace path: {$relative}\n");
            $fail++;
            continue;
        }

        if (preg_match('~^tests/(Catalog|Cataloging)(/|$)~', $relative) === 1) {
